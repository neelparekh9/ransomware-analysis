...
for result in analysis_results:
    ...
    if 'tcp_connections' in result['network']:
        connections_tcp_df = pd.concat([
            connections_tcp_df,
            pd.DataFrame(result['network']['tcp_connections']).assign(**{'Report Number': report_number}),
        ], ignore_index=True)

    if 'udp_connections' in result['network']:
        connections_udp_df = pd.concat([
            connections_udp_df,
            pd.DataFrame(result['network']['udp_connections']).assign(**{'Report Number': report_number}),
        ], ignore_index=True)

    if 'dns_request_details' in result['network']:
        dns_request_details_df = pd.concat([
            dns_request_details_df,
            pd.DataFrame(result['network']['dns_request_details']).assign(**{'Report Number': report_number}),
        ], ignore_index=True)
...
http_dns_df.to_excel("http_dns.xlsx")
connections_tcp_df.to_excel("tcp.xlsx")
connections_udp_df.to_excel("udp.xlsx")
dns_request_details_df.to_excel("dns_req.xlsx")
